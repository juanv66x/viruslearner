...
### 4.1.3 `vl_train` Database

In this training dataset, the emphasis is on modeling the persistence or suppression of viral load in patients, with the viral load in 2022 (`vl_2022`) as the outcome variable.

### 4.1.4 `cd_test` Database
...
### 4.1.5 `vl_test` Database

Similar to the previous test set, this dataset is used to evaluate the model’s predictive ability for viral load in 2022.
...
data("cd_fit1", package = "viruslearner")
cd_fit1
```
