...
#' @param type If TRUE, ensemble modeling is used; if FALSE, individual model building is used (default is TRUE).
...
#' cd_ens(outcome, traindata, viralvars, logbase, seed, repetitions, gridsize, TRUE, TRUE, TRUE)
#' }
cd_ens <-   function(outcome, traindata, viralvars, logbase, seed, repetitions, gridsize, mode = TRUE, type = TRUE, get_models = TRUE) {
  if (get_models) {
  ...
  # Handle the rest of the code for cases where get_models is not TRUE
  if (type == FALSE) {
    if (mode == TRUE) {
      return(
        dplyr::bind_rows(
